...
impl IO {
    fn create(debounce: Duration) -> IO {
    ...
    fn create_button(&mut self, pin: u8) -> Button {
    ...
    fn create_led(&self, yellow_pin: u8, green_pin: u8) -> Led {
        let yellow = Gpio::new()
            .expect("Failed to create Gpio")
            .get(yellow_pin)
            .expect("Failed to get pin")
    ...
    fn listen(mut self) -> Receiver<(u8, bool)> {
        let (tx, rx) = mpsc::channel();
        thread::spawn(move || {
            loop {
                let now = Instant::now();
